...
from TOOTpredictor import TOOTpredictor, collect_new_data_and_labels as collect_toot, retrain_toot_model, handle_feedback, ask_true_label, load_and_preprocess_image, save_results_to_google_sheets as toot_save_results_to_google_sheets
from main_audio import BeemoAudioProcessor
...
def trigger_event(bnb_result, qnq_result, toot_result=None):
    """
    Map the result from audio analysis to a Blynk event.
    """
    logger.debug(f"Triggering event for BNB result: {bnb_result}, QNQ result: {qnq_result}, TOOT result: {toot_result}")
    ...
    if toot_result == "Tooting Detected":
        logger.debug("Triggering notification: Queen Tooting Detected")
        trigger_notification("Queen Tooting Alert", "queen_tooting", "Queen tooting sound detected, indicating potential queen emergence or queen-queen competition.")
    elif toot_result == "No Tooting":
        logger.debug("No queen tooting detected")
        # Optionally add a notification for no tooting if needed

async def record_and_spectrogram(channels, api, output_box1=None, output_box2=None, output_box3=None):
    """
    Record audio, generate spectrogram, and perform predictions with optional output boxes
    """
    global last_predicted_spectrogram
    try:
        # Initialize BeemoAudioProcessor
        audio_processor = BeemoAudioProcessor()
        
        # Select device and record audio
        audio_processor.select_audio_device(0)  # Select first input device
        audio_path = audio_processor.record_audio(duration=10, channels=channels)
        
        if audio_path:
            # Generate spectrogram
            final_spectrogram_path = audio_processor.audio_to_spectrogram(audio_path)
            
            if final_spectrogram_path:
                gui_update_queue.put(('display_spectrogram', final_spectrogram_path))
                gui_update_queue.put(('update_filename_label', os.path.basename(final_spectrogram_path)))
                spectrogram_filenames.append(final_spectrogram_path) 
                gui_update_queue.put(('update_saved_spectrograms_list',))
                last_predicted_spectrogram = final_spectrogram_path
                ...
                # Perform predictions with optional output boxes
                if output_box1 is not None:
                    try:
                        output_box1.delete(1.0, tk.END)
                        bnb_result = await asyncio.to_thread(predict_and_display, final_spectrogram_path, output_box1)
                        logger.debug(f"BNB Result: {bnb_result}")
                    except Exception as bnb_error:
                        logger.error(f"BNB prediction error: {bnb_error}")

                if output_box2 is not None:
                    try:
                        output_box2.delete(1.0, tk.END)
                        qnq_result = await asyncio.to_thread(QNQpredictor, final_spectrogram_path, output_box2)
                        logger.debug(f"QNQ Result: {qnq_result}")
                    except Exception as qnq_error:
                        logger.error(f"QNQ prediction error: {qnq_error}")

                if output_box3 is not None:
                    try:
                        output_box3.delete(1.0, tk.END)
                        toot_result = await asyncio.to_thread(TOOTpredictor, final_spectrogram_path, output_box3)
                        logger.debug(f"TOOT Result: {toot_result}")
                    except Exception as toot_error:
                        logger.error(f"TOOT prediction error: {toot_error}")

                # Conversion and processing only if results exist
                ...
                bnb_result = convert_to_float(bnb_result or [0, 0.0])
                qnq_result = convert_to_float(qnq_result or [0, 0.0])
                toot_result = convert_to_float(toot_result or [0, 0.0])

                # Rest of the existing prediction logic remains the same
                # Prepare discord message, send to Blynk, etc.
                discord_message = {
                    "BNB Prediction": {
                        "File": os.path.basename(final_spectrogram_path),
                        "Predicted": "Bees Detected" if bnb_result[0] == 1 else "No Bees Detected",
                        "Confidence": float(bnb_result[1]) * 100,
                    },
                    "QNQ Prediction": {
                        "File": os.path.basename(final_spectrogram_path),
                        "Predicted": "Queen Detected" if qnq_result[0] == 1 else "No Queen Detected",
                        "Confidence": float(qnq_result[1]) * 100,
                    },
                    "TOOT Prediction": {
                        "File": os.path.basename(final_spectrogram_path),
                        "Predicted": "Tooting Detected" if toot_result[0] == 1 else "No Tooting",
                        "Confidence": float(toot_result[1]) * 100,
                    }
                }

                # Send the message to Discord
                logger.debug(f"Sending to Discord: {json.dumps(discord_message, indent=2)}")
                await send_discord_message(json.dumps(discord_message, indent=2), final_spectrogram_path)
                ...
                toot_prediction = "Tooting Detected" if toot_result[0] == 1 else "No Tooting"
                send_string_to_blynk(2, bnb_prediction)  # V2 for BNB
                send_string_to_blynk(3, qnq_prediction)  # V3 for QNQ
                send_string_to_blynk(4, toot_prediction)  # V4 for TOOT

                # Trigger event based on prediction
                trigger_event(bnb_prediction, qnq_prediction, toot_prediction)
...
async def record_and_spectrogram_worker(device_info):
    """
    Worker function for recording and processing audio with dynamic output box handling
    """
    channels, api = device_info

    try:
        # Dynamically retrieve output boxes from the global GUI context
        global output_box1, output_box2, output_box3
        
        # Check if output boxes are defined in the global scope
        output_boxes = []
        for box_name in ['output_box1', 'output_box2', 'output_box3']:
            if box_name in globals():
                output_boxes.append(globals()[box_name])
            else:
                logger.warning(f"{box_name} is not defined in the global scope")
        
        # Fallback to None if not enough output boxes are found
        while len(output_boxes) < 3:
        ...
        # Call record_and_spectrogram with dynamically retrieved output boxes
        await record_and_spectrogram(
            channels, 
            api, 
            output_box1=output_boxes[0], 
            output_box2=output_boxes[1], 
            output_box3=output_boxes[2]
        )

        # Analyze audio (optional, depending on your requirements)
        ...
        logger.error(f"Error in record_and_spectrogram_worker: {e}")
        # Optionally, send a notification or handle the error gracefully
        await notify_discord(f"Recording error: {e}")
